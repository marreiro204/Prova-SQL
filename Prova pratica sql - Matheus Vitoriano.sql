...
--- CRIANDO UMA CTE PARA  SABER FUNCIONÁRIOS DO TIPO VETREINÁRIO------

WITH FuncionariosVet AS (
    SELECT 
        Funcionarios.ID_Fun,
        Funcionarios.nome,
        Funcionarios.turno,
...
SELECT * 
FROM FuncionariosVet;


SELECT * 
FROM FuncionariosMasculinoVet;
...
BEGIN
	...
	------------- LOOP ----------
	---- ESSE LOOP 
	-- Loop para atualizar o turno de todos os funcionários que estão no turno da noite
	DECLARE @NovoTurno VARCHAR(7) = 'Tarde';
DECLARE @TurnoAntigo VARCHAR(7) = 'Noite';  
DECLARE @Id_Fun INT;


WHILE EXISTS (SELECT 1 FROM Tosadores WHERE turno = @TurnoAntigo)
BEGIN 
    SELECT TOP 1 @Id_Fun = ID_Fun
    FROM Tosadores
    WHERE turno = @TurnoAntigo;
